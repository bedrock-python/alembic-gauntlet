...
def _make_pats(
    ...
    suffixes: list[str],
    *,
    trailing_digit: bool = False,
) -> tuple[list[re.Pattern[str]], list[re.Pattern[str]]]:
    """Compile prefix and suffix patterns.

    Args:
        trailing_digit: When True, suffixes allow an optional trailing digit
            (for PostgreSQL partition naming, e.g. ``users_pkey1``).
    """
    prefix_pats = [re.compile(f"^{re.escape(p)}.*") for p in prefixes]
    digit = "\\d*" if trailing_digit else ""
    suffix_pats = [re.compile(f".*{re.escape(s)}{digit}$") for s in suffixes]
...
def validate_naming_results(
    ...
    """
    idx_prefix_pats, idx_suffix_pats = _make_pats(allowed_index_prefixes, allowed_index_suffixes, trailing_digit=True)
